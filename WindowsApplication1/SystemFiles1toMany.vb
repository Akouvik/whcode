...
Public Class SystemFiles1toMany
    Public Shared Sub SaveFile(ByVal tableName As String, ByVal Identifier As String, Optional ByVal SequenceNum As Integer = 0)
        ...
        If SequenceNum = 0 Then
            SequenceNum = 1 'in case this is the first file of this series being added
            If CheckFile(tableName, Identifier, SequenceNum) Then 'in case this is NOT the first file of this series being added
                cmd.CommandText = "Select Max(SequenceNum) from " & tableName & " where Identifier = " & Identifier & ";"
    ...
    Public Shared Function CheckFile(ByVal tableName As String, ByVal Identifier As String, Optional ByVal SequenceNum As Integer = 0) As Boolean
        Dim query As New SqlCommand
        If SequenceNum = 0 Then 'check for existence of ANY files from this series (by identifier)
            query.CommandText = "Select count(*) from " & tableName & " where Identifier = '" & Identifier & "';"
        Else 'check for existence of this SPECIFIC file
            query.CommandText = "Select count(*) from " & tableName & " where Identifier = '" & Identifier & "' and SequenceNum = " & SequenceNum & ";"
        End If
    ...
    Public Shared Sub OpenFile(ByVal tableName As String, ByVal Identifier As String, ByVal SequenceNum As Integer, ByVal outputFileName As String)
        ...
        cmd.CommandText = "Select FileExtension from " & tableName & " where Identifier = '" & Identifier & "' and SequenceNum = " & SequenceNum & ";"
        tempFileExt = cmd.ExecuteScalar().ToString
        'Grab the binary data from FileStream and load it into the SqlDataReader
        cmd.CommandText = "Select FileData from " & tableName & " where Identifier = '" & Identifier & "' and SequenceNum = " & SequenceNum & ";"
    ...
    Public Shared Sub DeleteFile(ByVal tableName As String, ByVal Identifier As String, ByVal SequenceNum As Integer)
        If CheckDelete() = True Then
            Dim cmd As New SqlCommand("Delete from " & tableName & " where Identifier = '" & Identifier & "' and SequenceNum = " & SequenceNum & ";", WarehouseShell.conn)
            cmd.ExecuteNonQuery()
        End If
    End Sub
