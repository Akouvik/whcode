Public Class TextBoxDatePast
    ...
    Public Sub setValue(ByVal formTable As DataSet, ByVal tableName As String, ByVal fieldName As String)
        If IsDate(formTable.Tables(tableName).Rows(0).Item(fieldName)) Then
            Text = Format(formTable.Tables(tableName).Rows(0).Item(fieldName), "MM-dd-yyyy").ToString 'Format date style before converting to string
        Else
            Clear()
        End If
    End Sub
    ...
    End Sub
    Private Sub TextBoxDate_Leave(sender As Object, e As EventArgs) Handles Me.Leave
    ...
    Private Sub Me_TypeValidationCompleted(ByVal sender As Object, ByVal e As TypeValidationEventArgs) Handles Me.TypeValidationCompleted
        If (Not e.IsValidInput) And Text <> "  /  /" Then
            errProvider.SetError(Me, "Must be valid date in the format mm/dd/yyyy.")
        Else
            ' Now that the type has passed basic type validation, enforce more specific type rules. 
            Dim UserDate As Date = CDate(e.ReturnValue)
            If (UserDate > Date.Now) Then
                e.Cancel = True
                errProvider.SetError(Me, "Must not be in the future.")
            Else
                errProvider.Clear()
            End If
        End If
